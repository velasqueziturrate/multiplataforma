...
module.exports = function (hookArgs, $injector, $testExecutionService) {
    if (isCLIVersionLowerThan6($injector)) {
        ...
        if ($testExecutionService && $testExecutionService.platform && !bundle) {
            ...
            let platformData = $platformsData.getPlatformData($testExecutionService.platform), projectFilesPath = path.join(platformData.appDestinationDirectoryPath, "app"), packageJsonPath = path.join(projectFilesPath, 'package.json'), packageJson = JSON.parse(fs.readFileSync(packageJsonPath).toString());
